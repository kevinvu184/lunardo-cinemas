...
function checkMail() {
  if (mailRegex.test(document.getElementById("cust-email").value)) {
    document.getElementById("cust-email").style.borderLeft =
      "palegreen 10px solid";
    return true;
  } else {
    document.getElementById("emailError").innerHTML =
      '<div class="form-error">Please enter valid mail</div>';
    document.getElementById("cust-email").style.borderLeft =
      "salmon 10px solid";
    return false;
  }
...
function checkName() {
  if (nameRegex.test(document.getElementById("cust-name").value)) {
    document.getElementById("cust-name").style.borderLeft =
      "palegreen 10px solid";
    return true;
  } else {
    document.getElementById("nameError").innerHTML =
      '<div class="form-error">Please enter valid name</div>';
...
function checkPhoneNumber() {
  if (phoneRegex.test(document.getElementById("cust-mobile").value)) {
    document.getElementById("cust-mobile").style.borderLeft =
      "palegreen 10px solid";
    return true;
  } else {
    document.getElementById("mobileError").innerHTML =
      '<div class="form-error">Please enter valid mobile phone</div>';
    document.getElementById("cust-mobile").style.borderLeft =
      "salmon 10px solid";
    return false;
  }
...
function checkCreditCardNumber() {
  if (creditCardRegex.test(document.getElementById("cust-card").value)) {
    document.getElementById("cust-card").style.borderLeft =
      "palegreen 10px solid";
    return true;
  } else {
    document.getElementById("cardError").innerHTML =
      '<div class="form-error">Please enter valid credit card</div>';
...
function checkExpirySelection() {
  if (
    isNaN(document.getElementById("cust-expiryMonth").value) ||
    isNaN(document.getElementById("cust-expiryYear").value)
  ) {
    document.getElementById("expiryError").innerHTML =
      '<div class="form-error">Please select expiry date for your card</div>';
    return false;
  } else {
    return true;
  }
...
function checkSelectSeat() {
  if (document.getElementById("finalPrice").textContent == "0.00 $") {
    document.getElementById("seatError").innerHTML =
      '<div class="form-error">Please choose seat before submit</div>';
    return false;
  } else {
    return true;
  }
...
function formValidate() {
  ...
  if (!checkMail()) countErrors++;
  if (!checkName()) countErrors++;
  if (!checkExpirySelection()) countErrors++;
  if (!checkCreditCardNumber()) countErrors++;
  if (!checkSelectSeat()) countErrors++;
  if (!checkPhoneNumber()) countErrors++;
...
function limitMonth() {
  ...
  if (yearSelected == yearAtTheMoment) {
    if (isNaN(monthSelectedLimit)) {
      ...
      document.getElementById("cust-expiryMonth").innerHTML = monthResult;
      let indexOfMonthSelected =
        monthSelectedLimit - (new Date().getMonth() + 1) + 1;
      document.getElementById("cust-expiryMonth").options[
        indexOfMonthSelected
      ].selected = true;
    }
  } else {
    if (isNaN(monthSelectedLimit)) {
...
function limitYear() {
  ...
  if (monthSelected < monthAtTheMoment) {
    if (isNaN(yearSelectedLimit)) {
      ...
      for (
        let j = new Date().getFullYear() + 1;
        j <= new Date().getFullYear() + MAXIMUM_YEAR_SPAN;
        ++j
      ) {
      ...
      document.getElementById("cust-expiryYear").innerHTML = yearResult;
      document.getElementById("cust-expiryYear").options[
        indexOfYearSelected
      ].selected = true;
    }
  } else {
    if (isNaN(yearSelectedLimit)) {
      ...
      yearResult += '<option value="YYYY">YYYY</option>';
      let indexOfYearSelected =
        yearSelectedLimit - new Date().getFullYear() + 1;
      for (
        let j = new Date().getFullYear();
        j <= new Date().getFullYear() + MAXIMUM_YEAR_SPAN;
        ++j
